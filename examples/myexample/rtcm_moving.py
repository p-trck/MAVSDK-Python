...
async def move_drone(drone):
    ...
    try:
        ...
        # 5회 반복
        for i in range(5):
            print(f"-- Moving cycle {i+1}/5")
            # 좌로 2미터
            await drone.offboard.set_position_ned(PositionNedYaw(0.0, 2.0, -2.0, 0.0))
...
async def process_rtcm(px4_handler, ntrip_client):
    """RTCM 데이터 처리 함수"""
    while True:
...
async def main():
    ...
    # PX4 드론 설정
    drone_address = "udp://:14540"  # 로컬 시뮬레이션의 경우
    ...
    try:
        ...
        # 드론 이동 태스크와 RTCM 데이터 처리 태스크를 동시에 실행
        movement_task = asyncio.create_task(move_drone(px4_handler.drone))
        rtcm_task = asyncio.create_task(process_rtcm(px4_handler, ntrip_client))
        
        # 두 태스크 중 하나라도 완료될 때까지 대기
        done, pending = await asyncio.wait(
            [movement_task, rtcm_task],
